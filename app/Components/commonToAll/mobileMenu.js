  ...
  return (
    <nav className='p-2 lg:hidden relative  '>
      <div className='flex justify-between items-center'>
        <Link href="/">
          <div className="h-auto w-14 overflow-hidden"><Image style={{ objectFit: "cover" }} height={200} width={200} src={logo.logo.url} alt='logo' /></div>
        </Link>
          ...
          {open && 
            nav && nav.length > 0 && nav.map((one, index) => {
              return <div className='border p-2'>
                <Link key={index} className='block ' href={one.link}>{capitalise(one.name)}</Link>
              </div>
            })}
