...
const Dashboard = ()=>{
    ...
    return (
        <div className=" h-full bg-gray-100 px-5 py-8">
            <DataDetail sites={sites?.length} blog={blog?.length} tags={tags?.length} category={categories?.length}/>
            <div className="grid grid-cols-1 md:grid-cols-2 my-2 gap-5">
                <div className="">
                    <StorageStatic site={sites?.length} blog={blog?.length} category={categories?.length} tag={tags?.length}/>
