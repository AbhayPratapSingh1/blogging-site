...
import { clearError, deleteNavigationRequest, singleNavigationRequest } from '../../../features/navigationSlice'
import { RiDeleteBinFill } from 'react-icons/ri'



export default function DeleteNavigation() {
  const { navId, siteId } = useParams()
  const dispatch = useDispatch()
  const singleNavigation = useSelector((state) => state.navigation.singleNavigation)
  const successMessage = useSelector((state) => state.app.message)
  const errorMessage = useSelector((state) => state.navigation.errorInNavigation)
  const navigate = useNavigate()
// console.log(navifatio);
  ...
  const deleteNavigation = (id) => {
    ...
    dispatch(deleteNavigationRequest(id))
  }
        ...
        <div className="w-[30rem] min-h-80 bg-red-50 rounded-xl p-2 ">
          <div className=" rounded-xl h-full flex items-center justify-center flex-wrap py-auto overflow-hidden">
            ...
            <div className="basis-full text-xl  font-semibold">
              <p className="text-center h-fit text-gray-800">ARE YOU SURE TO DELETE NAVIGATION <br/>"{singleNavigation.name}"</p>
