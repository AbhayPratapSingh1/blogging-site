...
import { deleteBlogRequest, clearError, singleBlogRequest } from '../../../features/blogSlice'
...
export default function DeleteBlog() {
  ...
  const resetMessage = ({ error }) => {
    dispatch(setMessage(""))
    dispatch(clearError())
    if (!error) navigate(`/sites/${siteId}/blogs`)
  }

  useEffect(() => {
    dispatch(singleBlogRequest(blogId))
  }, [dispatch, blogId])
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 text-center mb-2">
            Are you sure you want to delete <span className="font-medium">"{blog?.title}"</span>?
          </p>
          <p className="text-gray-400 text-sm text-center mb-6">This action cannot be undone.</p>
          
          <div className="flex gap-3 w-full">
            <Link to={`/sites/${siteId}/blogs`} className="flex-1">
