        ...
        <section>
            ...
            <div className=" rounded-xl">
                <table onClick={()=>{toggleTableMenu(-1)}} className=" table-fixed w-auto">
                    ...
                    <tbody>
                        {filterPost && filterPost.length > 0 && filterPost.map((one, index) => {
                            return (
                                <tr key={index} className="border-b h-10 border-gray-200 text-gray-600 text-sm">
                                    <td onClick={() => { toggleSelect(one) }} className="text-left"><input onChange={()=>{}} checked={isEntryCheck(one._id)} className="w-4 h-4 rounded border-black border text-center" type="checkbox" name="" id="" /></td>
                                    {header && header.length > 0 && header.map((each, index) => {
                                        const { accesserId, formatFunction, type } = each
                                        if (type === "boolean") {
                                            return <td key={index} className="text-left text-gray-400">{one[accesserId] ? "YES" : "NO"}</td>
                                        }
                                        return (
                                            <td key={index} className={`text-left ${index === 0 ? "text-gray-600" : (index === header.length - 1 ? "text-gray-500" : "text-gray-400")}`}>{formatFunction ? formatFunction(one[accesserId]) : one[accesserId]}</td>
                                        )
                                    })}
                                    <td className="relative w-12 h-full text-right">
                                        ...
                                        {index === tableMenu && <div onBlur={()=>{toggleTableMenu(-1)}} className="absolute z-50 border border-gray-400 -top-1 left-5 bg-gray-50 shadow-md rounded-md">
                                            <div className="flex flex-col items-start p-1 px-5">
                                                <Link to={`${editLink}/${one._id}`}> <div className=" border-gray-500 ">Edit</div></Link>
                                                <Link to={`${DeleteLink}/${one._id}`}><div className=" border-gray-500 ">Delete</div></Link>
                                            </div>
                                        </div>}
