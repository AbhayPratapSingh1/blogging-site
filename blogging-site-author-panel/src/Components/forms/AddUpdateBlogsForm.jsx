...
function AddUpdateBlogsForm({ edit }) {
    ...
    useEffect(() => {
        ...
        return () => {
            if (edit) {
                dispatch(setSingleBlog({}))
            }
        }
    }, [dispatch, siteId, blogId, edit]);
    
    const initialValues = {
        site: siteId,
        title: edit && singleBlog?.title ? singleBlog.title : "",
        description: edit && singleBlog?.description ? singleBlog.description : "",
        category: edit && singleBlog?.category ? singleBlog.category : "",
        tags: edit && singleBlog?.tags ? singleBlog.tags : "",
        metaTitle: edit && singleBlog?.metaTitle ? singleBlog.metaTitle : "",
        metaDescription: edit && singleBlog?.metaDescription ? singleBlog.metaDescription : "",
        metaKeywords: edit && singleBlog?.metaKeywords ? singleBlog.metaKeywords : "",
        faqHeading: edit && singleBlog?.faqHeading ? singleBlog.faqHeading : "",
        redirectUrl: edit && singleBlog?.redirectUrl ? singleBlog.redirectUrl : "",
        slug: edit && singleBlog?.slug ? singleBlog.slug : "",
        coverAlt: edit && singleBlog?.coverAlt ? singleBlog.coverAlt : "",
        faqs: edit && singleBlog?.faqs ? singleBlog.faqs : [],
        images: edit && singleBlog?.images ? singleBlog.images : {},
    }
                ...
                {({ values, setFieldValue, isSubmitting, touched, errors }) => (
                    <Form className='bg-gray-100 flex gap-x-20 gap-y-3 px-5 py-5 flex-wrap'>
                        ...
                        <div onClick={()=>{dispatch(toggleMultiSelectMenu(false))}} className=" h-10 w-96">
                            ...
                            <select value={values?.category} className='h-full w-full col-span-1 max-w-96 px-2 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500' onChange={(e) => setFieldValue('category', e.target.value)}>
                        ...
                        <div  onClick={()=>{dispatch(toggleMultiSelectMenu(false))}} className="w-full text-center ">
                            <button disabled={false} className=" px-4 py-2 rounded bg-blue-700 text-white  disabled:bg-gray-200 " type="submit"  >Submit</button>
                        </div>
                    </Form>
                )}
            </Formik>
