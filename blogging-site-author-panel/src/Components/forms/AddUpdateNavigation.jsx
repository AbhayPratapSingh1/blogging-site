...
import { addNewNavigationRequest, clearError, getAllNavigationRequest, singleNavigationRequest, updateNavigationRequest } from "../../features/navigationSlice";
...
export default function AddUpdateNavigation({ edit }) {
    ...
    const initialValues = {
        ...
        position: edit ? singleNav?.position?.toString() : "",
    }
                ...
                {({ values, isSubmitting }) => {
                    return (
                        <Form className="p-10 bg-gray-100">
                            <FieldBox value={values.name} name="name" label="Navigation Name " />
                            <FieldBox value={values.link} name="link" label="Link " />
                            <FieldBox value={values.position} name="position" label="Position " />
